        ...
        <div class="flex flex-row space-x-3">
            <div>
                <div class="flex flex-col space-y-3 pb-5 max-w-2xl items-center">
                    ...
                    <div class="flex flex-row space-x-3 justify-center">
                        ...
                        <div>
                            <input type="checkbox" v-model="preserveThinking" class="ring-0">&nbsp;Preserve
                            thinking</input>
                        </div>
                    </div>
                    <div class="flex flex-row space-x-3">
                    ...
                    <div class="w-full flex justify-center">
                        <div class="flex flex-col space-y-2 mt-3">
                            ...
                            <sw-switch v-model:value="switchPropagateModel" class="text-sm">Use this model for all
                                subagents</sw-switch>
                            <sw-switch v-model:value="switchPropagateIp" class="text-sm">Use this inference params for
                                all
                                subagents</sw-switch>
                            <div class="flex flex-row space-x-2 pt-2 items-center">
                                ...
                                <div>
                                    <select v-model="backend" :required="true"
                                        ...
                                        <option v-for="b in Object.keys(state.backends)"
                                            :selected="uistate.backend == b" :value="b">
                                            {{ b }}
                                        </option>
                                    </select>
                                </div>
            ...
            <div class="max-w-64">
                ...
                <div class="flex flex-wrap gap-2 justify-around">
                    ...
                    <button v-for="preset in state.samplingPresets" class="btn soft"
                        @click="applySamplingPreset(preset)">{{
                            preset.name }}</button>
                </div>

            </div>
        </div>
        <div class="flex flex-row justify-center pt-3 space-x-2">
            <button class="btn font-semibold hover:prim py-1 text-sm" :disabled="!isValid"
                @click="useAgentSettings()">Use agent settings</button>
            <button class="btn success py-1 text-sm" :disabled="!isValid" @click="saveAgentsSettings()">Save
                agent
                settings</button>
        </div>
    </div>
