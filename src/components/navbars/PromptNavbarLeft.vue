<template>
    <div class="flex flex-row text-light">
        ...
        <button class="btn pl-1! pr-1! flex flex-row space-x-2" v-else-if="uistate.viewMode == 'text'"
            @click="toggleTextViewMode('raw')">
            <TextFormatIcon height="32" width="32" class="mt-1"></TextFormatIcon>
        </button>
        <button class="btn pl-1! pr-1! flex flex-row space-x-2" v-else @click="toggleTextViewMode('markdown')">
            <TextIcon height="32" width="32" class="mt-1"></TextIcon>
        </button>
        <button class="btn pl-1! pr-1! flex flex-row space-x-2" v-if="uistate.historyViewMode == 'plain'"
            @click="toggleHistoryView()">
            <HistoryFull height="32" width="32" class="mt-1"></HistoryFull>
        </button>
        <button class="btn pl-1! pr-1! flex flex-row space-x-2" v-else>
            <HistoryCompact height="32" width="32" class="mt-1" @click="toggleHistoryView()"></HistoryCompact>
        </button>
    </div>
</template>
...
function toggleHistoryView() {
    if (uistate.value.historyViewMode == "plain") {
        uistate.value.historyViewMode = "toplevel"
    } else {
        uistate.value.historyViewMode = "plain"
    }
}
